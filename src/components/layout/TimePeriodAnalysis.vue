<template>
  <div class="analysis-module">
    ...
    <template v-else>
      ...
      >
        <el-table-column prop="period" label="时间段" width="180" />
  ...
  setup() {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        // 生成时间段数据
        const totalMarketValue = accountData.total_market_value;
        const baseValue = totalMarketValue * 0.8; // 基准值，用于计算增长率

        timeData.value = [
          {
            period: '近一周',
            totalAssets: totalMarketValue,
            returnRate: accountData.positions.reduce((sum, pos) => sum + pos.daily_return, 0) / accountData.positions.length,
            growthRate: ((totalMarketValue - baseValue) / baseValue * 100).toFixed(2)
          },
          {
            period: '近一月',
            totalAssets: totalMarketValue * 1.05,
            returnRate: accountData.positions.reduce((sum, pos) => sum + pos.daily_return, 0) / accountData.positions.length * 4,
            growthRate: ((totalMarketValue * 1.05 - baseValue) / baseValue * 100).toFixed(2)
          },
          {
            period: '近三月',
            totalAssets: totalMarketValue * 1.1,
            returnRate: accountData.positions.reduce((sum, pos) => sum + pos.daily_return, 0) / accountData.positions.length * 12,
            growthRate: ((totalMarketValue * 1.1 - baseValue) / baseValue * 100).toFixed(2)
          },
          {
            period: '近半年',
            totalAssets: totalMarketValue * 1.15,
            returnRate: accountData.positions.reduce((sum, pos) => sum + pos.daily_return, 0) / accountData.positions.length * 24,
            growthRate: ((totalMarketValue * 1.15 - baseValue) / baseValue * 100).toFixed(2)
          },
          {
            period: '近一年',
            totalAssets: totalMarketValue * 1.2,
            returnRate: accountData.positions.reduce((sum, pos) => sum + pos.daily_return, 0) / accountData.positions.length * 48,
            growthRate: ((totalMarketValue * 1.2 - baseValue) / baseValue * 100).toFixed(2)
          }
        ];

        console.log('处理后的数据:', timeData.value);
    ...
    const initChart = () => {
      ...
      const option = {
        ...
        xAxis: {
          type: 'category',
          data: timeData.value.map(item => item.period),
          axisLabel: {
            color: '#ffffff'
          }
        },
        ...
        series: [
          ...
          },
          {
            ...
            type: 'line',
            yAxisIndex: 1,
            data: timeData.value.map(item => item.returnRate),
            itemStyle: {
              color: '#FFD700'
            }
          },
          {
            ...
            type: 'line',
            yAxisIndex: 1,
            data: timeData.value.map(item => item.growthRate),
            itemStyle: {
              color: '#32CD32'
            }
          }
