...
export default {
  ...
  data() {
    return {
      activeMenu: 'asset',
      accountId: '40000326', // 当前选中的账户ID
      availableAccounts: [], // 可用的账户列表
      riskThresholdData: [
    ...
    async fetchAvailableAccounts() {
      try {
        ...
        if (accountData && accountData.accounts && accountData.accounts.length > 0) {
          this.availableAccounts = accountData.accounts;

          // 如果没有选中账户，默认选择第一个
          if (!this.accountId && this.availableAccounts.length > 0) {
            this.accountId = this.availableAccounts[0].account_id;
          }

          console.log('可用账户列表:', this.availableAccounts);
        } else {
