          ...
          <div class="content-panel glass-panel chart-panel">
            ...
            <div class="panel-content">
              <div style="margin-bottom:8px;display:flex;gap:8px;align-items:center;">
                ...
                <!-- 刷新按钮 -->
                <el-button size="small" type="primary" @click="refreshAll" :loading="isRefreshing">
                  {{ isRefreshing ? '刷新中...' : '刷新' }}
                </el-button>

                <!-- 后端状态指示器 -->
                <div v-if="isBackendUnavailable" class="backend-status">
                  <span class="status-text">后端未连接</span>
                </div>
              </div>

              <!-- 数据真实性指示器 -->
              <DataAuthenticityIndicator :chart-data="getChartData()" />
...
import DataAuthenticityIndicator from '@/components/layout/DataAuthenticityIndicator.vue';
...
export default {
  name: 'ComparisonPage',
  components: { ComparisonChart, RiskThreshold, ComparisonTable, RiskWarning, DataAuthenticityIndicator },
  data() {
    return {
      activeMenu: 'asset',
      isRefreshing: false,
  ...
  computed: {
    ...
    isBackendUnavailable() {
      return this.availableAccounts.length === 1 &&
             this.availableAccounts[0]?.account_id === '连接失败';
    }
  },
    methods: {
    ...
    getChartData() {
      ...
      switch (this.activeMenu) {
        case 'asset':
          return { type: 'asset', accountId: this.accountId };
        case 'time':
          return { type: 'time', accountId: this.accountId };
        case 'region':
          return { type: 'region', accountId: this.accountId };
        default:
          return { type: 'unknown', accountId: this.accountId };
      }
    },
    ...
    getAccountLabel(account) {
      ...
      const typeText = account.account_type === 2 ? '股票' : '其他';
      return `${account.account_id} (${typeText})`;
    },

        // 获取可用账户列表
    async fetchAvailableAccounts() {
      try {
        console.log('正在获取可用账户列表...');
        const accountData = await fetchAccountInfo();

        if (accountData && accountData.accounts && accountData.accounts.length > 0) {
          setAvailableAccounts(accountData.accounts);
          console.log('可用账户列表:', accountData.accounts);
        } else {
          console.warn('没有找到可用账户');
        ...
        if (error.code === 'ERR_NETWORK' || error.message.includes('Failed to fetch')) {
          console.warn('后端连接失败，无法获取真实账户信息');
          // 可以在这里设置一个默认的占位账户，但标记为不可用
          setAvailableAccounts([{
            account_id: '连接失败',
            account_type: 0,
            disabled: true
    ...
    async refreshAll() {
      ...
      try {
        // 重新获取账户列表
        await this.fetchAvailableAccounts();
