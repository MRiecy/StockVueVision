...
export default {
  ...
  methods: {
    ...
    safeSeries(series) {
      ...
      // 如果有真实账户数据，使用前3只股票的数据
      let seriesData = [];
      if (this.accountData && this.accountData.positions && this.accountData.positions.length > 0) {
        const positions = this.accountData.positions.slice(0, 3); // 取前3只股票
        seriesData = positions.map((position, index) => {
          const colors = ['#40e0ff', '#ff6b6b', '#feca57'];
          const names = ['股票A', '股票B', '基准指数'];
          return {
            name: `${position.stock_code || names[index]}`,
            type: 'line',
            data: generateMockReturnData(), // 暂时使用模拟收益率数据
            smooth: true,
      ...
      return {
        ...
        legend: { data: seriesData.map(s => s.name), textStyle: { color: '#ffffff', fontSize: 12 }, bottom: '5%' },
        ...
        series: this.safeSeries(seriesData)
      };
    },
