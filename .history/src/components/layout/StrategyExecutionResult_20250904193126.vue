...
export default {
  ...
  setup() {
    ...
    const fetchExecutionResult = async () => {
      try {
        const { getCurrentAccountId } = await import('@/utils/accountStore.js');
        const accountId = getCurrentAccountId();
        const mock = 0; // 强制真实数据
        const url = accountId ? `http://localhost:8000/api/execution-result/${accountId}/` : 'http://localhost:8000/api/execution-result/';
        const response = await axios.get(url, { params: { account_id: accountId || undefined, mock } });
        keyMetrics.value = response.data.keyMetrics;
      } catch (error) {
        console.error('获取策略执行结果失败：', error);
        throw new Error(`获取策略执行结果失败: ${error.message}`);
      }
    };
