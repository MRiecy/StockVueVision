...
export default {
  ...
  setup(props) {
    ...
    const initCategoryChart = () => {
      ...
      // 检查是否已有图表实例，避免重复初始化
      let chart = echarts.getInstanceByDom(categoryChart.value);
      if (!chart) {
    ...
    const initTimeChart = () => {
      ...
      // 检查是否已有图表实例，避免重复初始化
      let chart = echarts.getInstanceByDom(timeChart.value);
      if (!chart) {
    ...
    const initRegionChart = () => {
      ...
      // 检查是否已有图表实例，避免重复初始化
      let chart = echarts.getInstanceByDom(regionChart.value);
      if (!chart) {
        chart = echarts.init(regionChart.value);
      }
      chart.setOption({
        title: {
          text: '地区对比分析',
