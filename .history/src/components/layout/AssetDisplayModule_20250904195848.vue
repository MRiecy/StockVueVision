...
import { fetchAccountInfo, fetchAccounts, getActiveAccount, setActiveAccount } from '@/api/accountApi.js';
...
onMounted(async () => {
  try {
    // 先获取账户列表与默认ID
    const listResp = await fetchAccounts();
    const list = Array.isArray(listResp?.accounts) ? listResp.accounts : [];
    accounts.value = list;

    // 获取后端"激活账户"优先，其次 fallback 到 default_account_id，再其次取列表第一个
    const active = await getActiveAccount().catch(() => null);
    if (active?.account_id) {
      selectedAccount.value = active.account_id;
    } else if (listResp?.default_account_id) {
      selectedAccount.value = listResp.default_account_id;
    } else if (accounts.value.length > 0) {
      selectedAccount.value = accounts.value[0].account_id;
    }

    // 再拉取该账户的详细信息
    const data = await fetchAccountInfo(selectedAccount.value);
    accountDetail.value = data;
    if (data && data.accounts) {
      // 注意：这里 data.accounts 很可能只返回当前账户的详情，勿覆盖原 accounts 列表
      const enriched = data.accounts.map((account) => {
      ...
      // 将 enriched 合并回原 accounts 列表（按 account_id 匹配更新）
      const map = new Map(accounts.value.map(a => [a.account_id, a]));
      enriched.forEach(a => map.set(a.account_id, { ...map.get(a.account_id), ...a }));
...
const handleAccountChange = async (accountId) => {
  ...
  try {
    ...
    console.error('设置激活账户失败:', e);
  }
  // 通知其他模块（图表/结果）重载
  window.dispatchEvent(new CustomEvent('active-account-changed', { detail: { account_id: accountId } }));
  // 同步拉取该账户的详情以刷新表格
  try {
    const data = await fetchAccountInfo(); // 不带参数，后端将使用激活账户
    accountDetail.value = data;
  } catch (e) {
    console.error('切换账户后获取详情失败:', e);
  }
};
