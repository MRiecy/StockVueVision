<template>
  <div class="data-authenticity-indicator compact">
    <!-- 精简版：只显示状态徽章和验证按钮 -->
    <div class="indicator-compact">
      <div class="status-badge" :class="statusClass">
        {{ statusText }}
      </div>
      <button @click="verifyData" class="verify-btn-compact" :disabled="isVerifying">
        {{ isVerifying ? '...' : '验证' }}
      </button>
    </div>
...
<style scoped>
/* 精简版样式 */
.data-authenticity-indicator.compact {
  margin: 5px 0;
}
...
.verify-btn-compact:hover:not(:disabled) {
  background: rgba(64, 224, 255, 0.3);
}

.verify-btn-compact:disabled {
