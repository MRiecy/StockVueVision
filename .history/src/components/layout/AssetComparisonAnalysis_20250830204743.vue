    ...
    const fetchTimeData = async () => {
      ...
      try {
        ...
        if (sourceData.length > 0) {
          ...
          // 如果没有时间段数据，从账户数据生成模拟数据
          console.log('没有时间段数据，从账户数据生成模拟数据...');
          const accountData = await fetchAccountInfo();
          if (accountData && accountData.accounts && accountData.accounts.length > 0) {
            const account = accountData.accounts[0];
            const totalAsset = account.total_asset || 0;
            ...
            timeData.value = [
              {
                timePeriod: `${currentYear - 2}`,
                totalAssets: Math.round(totalAsset * 0.85),
                returnRate: '5.2%',
                growthRate: '8.5%'
              },
              {
                timePeriod: `${currentYear - 1}`,
                totalAssets: Math.round(totalAsset * 0.92),
                returnRate: '6.8%',
                growthRate: '12.3%'
              },
              {
        ...
        if (regionResponse && regionResponse.region_data) {
          ...
          if (accountData && accountData.accounts && accountData.accounts.length > 0) {
            ...
            regionData.value = Object.entries(regionStats).map(([region, stats]) => ({
              ...
              returnRate: '5.2%', // 模拟收益率
              investmentRate: totalAsset > 0 ? ((stats.totalAssets / totalAsset) * 100).toFixed(1) + '%' : '0%'
            }));
          } else {
            regionData.value = [];
          }
