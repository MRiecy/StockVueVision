// src/api/accountApi.js
import axios from 'axios'

// 创建一个axios实例
const api = axios.create({
  baseURL: 'http://localhost:8000',  // 添加后端服务器地址
  timeout: 30000, // 增加超时时间到30秒
  headers: {
    'Content-Type': 'application/json',
    'Accept': 'application/json'
  }
});

/**
 * 从后端获取账户资产及持仓数据
 * @returns {Promise<Object>} 返回后端的 JSON 数据，例如 { accounts: [...] }
 */
export async function fetchAccountInfo(accountId) {
  try {
    console.log('请求账户信息API...', accountId);
    const response = await api.get('/api/account-info/', { params: accountId ? { account_id: accountId } : {} });
    console.log('账户信息API响应:', response.status);
    return response.data;
  } catch (error) {
    console.error('账户信息API错误:', error);
    if (error.response) {
      console.error('响应状态:', error.response.status);
      console.error('响应数据:', error.response.data);
    } else if (error.request) {
      console.error('请求已发送但未收到响应');
    } else {
      console.error('请求设置错误:', error.message);
    }

    throw new Error(`API请求失败: ${error.message}`);
  }
}

/**
 * 从账户信息获取资产类别数据
 * 此API用于账户总览页面的资产分类展示
 */
export async function fetchAssetCategoryData() {
  try {
    const response = await api.get('/api/asset-category/');
    return response.data;
  } catch (error) {
    console.error('获取资产类别数据失败:', error);
    throw new Error(`API请求失败: ${error.message}`);
  }
}

/**
 * 从账户信息获取地区分布数据
 * 此API用于账户总览页面的地区分布展示
 */
export async function fetchRegionDataFromBackend() {
  try {
    const response = await api.get('/api/region-data/');
    return response.data;
  } catch (error) {
    console.error('获取地区分布数据失败:', error);
    throw new Error(`API请求失败: ${error.message}`);
  }
}

/**
 * 从账户信息获取时间序列数据
 * 此API用于账户总览页面的时间序列展示
 */
export async function fetchTimeDataFromBackend(params) {
  try {
    const response = await api.get('/api/time-data/', { params });
    return response.data;
  } catch (error) {
    console.error('获取时间序列数据失败:', error);
    throw new Error(`API请求失败: ${error.message}`);
  }
}

/**
 * 列出所有账户并提供默认账户ID
 * 响应示例：{ accounts: [{ account_type, account_id }...], default_account_id: '40321422' }
 */
export async function fetchAccounts() {
  try {
    const response = await api.get('/api/accounts/');
    return response.data;
  } catch (error) {
    console.error('获取账户列表失败:', error);
    throw new Error(`API请求失败: ${error.message}`);
  }
}

/** 获取当前激活账户ID */
export async function getActiveAccount() {
  try {
    const res = await api.get('/api/accounts/active/');
    return res.data; // { account_id: 'xxxx' }
  } catch (e) {
    console.error('获取激活账户失败:', e);
    throw e;
  }
}

/** 设置当前激活账户ID */
export async function setActiveAccount(accountId) {
  try {
    const res = await api.post('/api/accounts/active/', { account_id: accountId });
    return res.data;
  } catch (e) {
    console.error('设置激活账户失败:', e);
    throw e;
  }
}

